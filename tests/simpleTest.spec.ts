...
test.describe('Datamaran practical exercise', () => {
    test('should navigate to Page2 and see the same values in dropdown', async ({ page }) => {
        // Precondition as despicted in exercise
        ...
        for (const tr of await page.locator("//table[@id='issue-bucket-mapping']//tr[td[@class='issue-col']]").all()){
        ...
        await expect(allDropdownItems, `Dropdown item quantity doesnt match expectations. Got ${page1count} in page1 and found ${allDropdownItems.length} in page2's dropsown`).toHaveLength(page1count);

        // iterate all values and check against those in page1
        for (const divItem of allDropdownItems){
